...
// 调试消息发送的时间，这里设置为0，代表默认不开启调试
#define GCS_DEBUG_SEND_MESSAGE_TIMINGS 0

// macros used to determine if a message will fit in the space available.
// 当MAVLink通道的缓存空间不足时调用的函数
// 检查给定MAVLink通道是否有足够的空间发送指定最大负载长度的消息
...
// output
// 检查是否有足够的空间发送特定ID的MAVLink消息，并在空间不足时直接返回false
